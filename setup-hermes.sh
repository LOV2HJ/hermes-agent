...
echo ""
if is_termux; then
    echo "  1. Configure a local LLM provider:"
    echo "     scripts/termux-local-llm.sh lmstudio <model-name>"
    echo "     scripts/termux-local-llm.sh ollama <model-name>"
    echo "     scripts/termux-local-llm.sh llamacpp <model-name> [http://127.0.0.1:8080/v1]"
    echo ""
    echo "  2. Or run the setup wizard for cloud/API-key providers:"
    echo "     hermes setup"
    echo ""
